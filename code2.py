...
# Instantiating the classification algorithms
    ...
    clf_list_temp = deepcopy(clf_list)
    ...
    for ii in range(len(clf_list)):
        for jj in range(len(clf_list)):
            for kk in tqdm(range(n_tests)):
                ...
                X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=0.34, random_state=randint)
